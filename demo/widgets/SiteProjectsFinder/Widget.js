define([
    ...
    return declare([BaseWidget], {
        ...
        _rangeSliderChangeTimer: null, // Timer to control buffer creation on slider change
        ...
        _rangeSlider: null, // Horizontal slider instance
        ...
        _initReverseGeocoder: function() {
            //If any geocoding service is available in org then only reverse geocoding will be performed,
            //also if org has multiple geocodeing services only first will be considered for reverse geocoding
            if (this.config.helperServices && this.config.helperServices.geocode &&
                ...
                this._locatorInstance = new Locator(this.config.helperServices.geocode[0].url);
        ...
        _onLocationToAddressComplete: function(result) {
            ...
            if (result.address && result.address.address) {
                ...
                if (this._searchInstance) {
                    this._searchInstance.setSearchText(result.address.address.Match_addr);
                }
        ...
        _sliderChange: function(value) {
            ...
            if (this._searchedLocation) {
                this._loading.show();
                // this._yearDefinitionExpression();
                this._sliderChangeTimer = setTimeout(lang.hitch(this, this._createBuffer), 700);
            }
        },
        /**
         * Sets definiton expression and year values
         * @memberOf widgets/SiteProjectsFinder/Widget
         **/
        _yearFilter: function() {
            ...
            for (var i in ftures) {
                year = Number(ftures[i].attributes.YEAR);
                if (year != 0) {
                    years.push(year);
            ...
            if (yearNode) {
                domAttr.set(yearNode, "innerHTML", textTemplate);
            }
            if (minNodeValue) {
                domAttr.set(minNodeValue, "placeholder", min);
            }
            if (maxNodeValue) {
                domAttr.set(maxNodeValue, "placeholder", max);
            }

        },

        _yearDefinitionExpression: function() {
            var minNodeValue, maxNodeValue, filter, placeholderYearMin, placeholderYearMax, yearMinVal, yearMaxVal;
            // Template Strings, notice the use of use back-ticks
            minNodeValue = query(".UCBYearMinValue", this.domNode)[0];
            this._yearMinVal = Number(domAttr.get(minNodeValue, "value"));
            maxNodeValue = query(".UCBYearMaxValue", this.domNode)[0];
            this._yearMaxVal = Number(domAttr.get(maxNodeValue, "value"));
            placeholderYearMin = query(".UCBYearMinValue")[0];
            placeholderYearMax = query(".UCBYearMaxValue")[0];
        ...
        // /**
        //  * This function create horizontal range slider and set minimum maximum value of slider
        //  * @memberOf widgets/SiteProjectsFinder/Widget
        //  **/
        // // ************** ULISES ESTUVO AQUI ***********
        ...
        //         maximum: max,
        //         intermediateChanges: true,
        //         discreteValues: new Set(years).size,
        //         style: "width:318px;",
        //         showButtons: true,
        //     }, "rangeSlider");
        //     // on change event of range slider
        //     this.own(this._rangeSlider.on("change", lang.hitch(this, this._anotherdummyfunction)));
        // },

        // _dummySliderChange: function() {
        //     var lowRange, highRange, projectsfilter, gLayer, gLayer2;
        //     // console.log('pos', this._rangeSlider.value[0], this._rangeSlider.value[1]);
        //     lowRange = this._rangeSlider.value[0];
        //     highRange = this._rangeSlider.value[1];
        //     // Template Strings, notice the use of use back-ticks
        ...
        //     // this.map.addLayer(projects);
        //     // console.log('prom', prom);
        //     gLayer = this.map.getLayer("graphicsLayer3");
        //     if (this._mapClickEvt != null) {
        //         // this._onMapClick(this._mapClickEvt);
        //         gLayer2 = this.map.getLayer("graphicsLayer3");
        //         // resetting evt value
        //         console.log('evt after if', this._mapClickEvt);
        //         console.log('graphicsLAyer3', gLayer);
        //         console.log('graphicsLAyer3-2', gLayer2);
        //         console.log('projects', projects);
        //         console.log('pos after if', lowRange, highRange);
        //     }


        // },

        // _anotherdummyfunction: function() {
        //     if (this._rangeSliderChangeTimer) {
        //         clearTimeout(this._rangeSliderChangeTimer);
        //     }
        //     this._rangeSliderChangeTimer = setTimeout(lang.hitch(this, this._dummySliderChange), 500);
         ...
         * @memberOf widgets/NearMe/Widget
         */
        _setBufferSliderVisiblity: function(selectedFeature) {
            var hideHorizontalSliderFlag = true,
                itemListMainContainer, horzontalSliderNode;
